...
def lambda_handler(event, context):
    ...
    instances = ec2.instances.filter(Filters=[{'Name':'tag:TeamOwner', 'Values':['TelecomLab']},{'Name':'tag:AutoStop', 'Values':['true']},{'Name': 'instance-state-name', 'Values': ['running','pending','rebooting']}])
    ...
    if len(scheduled_instances) > 0:
        response = client.stop_instances(InstanceIds=scheduled_instances)
